...
from common import fresh_output_directory
sys.path.insert(0, str(PROJECT / "repro/vendor/potts_tail"))
from spectral_tail_core import complete_retained_slices

DENSE_POTTS_BUDGET_BYTES = 512 * 1024**2


def check_dense_potts_budget(L: int, budget: int = DENSE_POTTS_BUDGET_BYTES) -> int:
    """Conservative eight dense arrays plus transition/index workspace, before allocation."""
    if not isinstance(L, int) or L < 2 or L > 12 or budget <= 0:
        raise ValueError("full Potts requires integer 2 <= L <= 12 and a positive budget")
    dim = 3**L
    required = 8 * 8 * dim**2 + 128 * L * dim
    if required > budget:
        raise MemoryError(f"legacy full Potts uses dense intermediates: {required} bytes exceed {budget}; use k0")
    return required


def bracketed_prg_root(f, h_min: float, h_max: float) -> float:
    if not (np.isfinite(h_min) and np.isfinite(h_max) and h_min < h_max):
        raise ValueError("PRG requires a finite increasing bracket")
    grid = np.linspace(h_min, h_max, 21)
    values = np.asarray([f(float(x)) for x in grid])
    if not np.all(np.isfinite(values)):
        raise RuntimeError("nonfinite PRG gap residual")
    ...
    raise RuntimeError("PRG search has no sign-changing bracket; no critical-field estimate produced")


def retained_level_weights(ev, amplitudes2, E0, nominal, tolerance):
    """Discard an incomplete boundary group, with a guard for every retained prefix."""
    groups = complete_retained_slices(ev, nominal, tolerance)
    weights = [float(np.sum(amplitudes2[a:b]) / (np.mean(ev[a:b]) - E0)**2)
               for a, b in groups if np.mean(ev[a:b]) - E0 > tolerance]
    if not weights:
        raise RuntimeError("cutoff contains no complete excited group")
    return np.asarray(weights), groups
...
def hybrid_level_projector_kf(H: sp.spmatrix, dH: sp.spmatrix,
                              n_low_energy_eigenpairs: int = 96,
                              tol_eig: float = 1e-9,
                              tol_group: float = 1e-7,
                              rtol_solve: float = 1e-9,
                              ncv_factor: int = 4,
                              max_solve_residual: float = 1e-6) -> dict:
    ...
    dH = dH.tocsr()
    dim = H.shape[0]
    if H.shape != dH.shape or H.shape[1] != dim or dim < 4:
        raise ValueError("hybrid response requires matching square matrices of dimension >= 4")
    if np.iscomplexobj(H.data) or np.iscomplexobj(dH.data):
        raise ValueError("this response kernel supports real symmetric inputs only")
    if not np.all(np.isfinite(H.data)) or not np.all(np.isfinite(dH.data)):
        raise ValueError("nonfinite Hamiltonian or perturbation")
    if n_low_energy_eigenpairs < 2 or ncv_factor < 2 or any(
            not np.isfinite(t) or t <= 0 for t in (tol_eig, tol_group, rtol_solve, max_solve_residual)):
        raise ValueError("invalid cutoff, Krylov factor or tolerance")
    ...
    ncv0 = min(dim, max(60, ncv_factor * min(n_low_energy_eigenpairs, dim - 2)))
    ...
    if not np.all(np.isfinite(ev0)) or not np.all(np.isfinite(ec0)) or gap <= tol_group:
        raise RuntimeError("nonfinite or numerically degenerate ground state")
    ...
    if info != 0 or not np.all(np.isfinite(y)):
        raise RuntimeError(f"MINRES did not return a finite converged response: info={info}")
    y = y - float(psi0 @ y) * psi0
    P2 = float(y @ y)
    solve_res = float(np.linalg.norm(A @ y - b) / max(np.linalg.norm(b), 1e-300))

    if not np.isfinite(P2) or P2 <= 0 or not np.isfinite(solve_res) or solve_res > max_solve_residual:
        raise RuntimeError(f"projected response rejected: P2={P2}, relative residual={solve_res}")

    nominal = min(n_low_energy_eigenpairs, dim - 2)
    n_eigenpairs_requested = nominal + 1  # guard eigenpair, not another retained level
    ncv_low = min(dim, max(80, ncv_factor * n_eigenpairs_requested + 1))
    ev, ec = spla.eigsh(
        H, k=n_eigenpairs_requested, which="SA", tol=tol_eig,
        ncv=ncv_low, v0=v0,
    )
    order = np.argsort(ev)
    ev = ev[order]
    ec = ec[:, order]
    if abs(float(ev[0]) - E0) > tol_group:
        raise RuntimeError("ground and retained eigensolves disagree on the lowest energy")
    amps2 = np.abs(ec.T @ b) ** 2
    if not np.all(np.isfinite(ev)) or not np.all(np.isfinite(ec)):
        raise RuntimeError("nonfinite retained eigensystem")
    xs, groups = retained_level_weights(ev, amps2, E0, nominal, tol_group)
    P4 = float(np.sum(xs**2))
    KF = P4 / P2**2

    # Low-level convergence diagnostic: recompute P4 using the first half.
    m_half = max(2, nominal // 2)
    xs_half, half_groups = retained_level_weights(ev, amps2, E0, m_half, tol_group)
    ...
    return {
        ...
        "maximum_solve_residual": max_solve_residual,
        "nominal_eigenpairs": nominal,
        "solver_eigenpairs": n_eigenpairs_requested,
        "guard_eigenpairs": 1,
        "retained_groups": groups,
        "half_retained_groups": half_groups,
        "n_levels": len(xs),
        "x": np.sort(xs)[::-1],
    }
...
def build_nnn_tfim_components(L: int, j2: float) -> tuple[sp.csr_matrix, sp.csr_matrix]:
...
def build_nnn_tfim(L: int, h: float, j2: float) -> tuple[sp.csr_matrix, sp.csr_matrix]:
    """H = -sum ZZ_1 - J2 sum ZZ_2 - h sum X, with PBC; dH/dh=-sum X."""
    Hzz, X = build_nnn_tfim_components(L, j2)
    H = (Hzz - h * X).tocsr()
    dH = (-X).tocsr()
    return H, dH
...
def parity_gap_nnn(L: int, h: float, j2: float, tol: float = 1e-10) -> dict[str, float]:
    """Build both k=0 parity sectors and return their lowest-energy gap."""
    even = build_nnn_tfim_sector_components_k0_parity(L, j2, +1)
    odd = build_nnn_tfim_sector_components_k0_parity(L, j2, -1)
    return parity_gap_nnn_from_components(even, odd, h, tol)


def build_nnn_tfim_sector_components_k0_even(L: int, j2: float) -> tuple[sp.csr_matrix, sp.csr_matrix, int]:
...
def build_nnn_tfim_sector_k0_even(L: int, h: float, j2: float) -> tuple[sp.csr_matrix, sp.csr_matrix, int]:
    ...
    exactly the restriction of
        ...
        H = -sum_i Z_i Z_{i+1} - J2 sum_i Z_i Z_{i+2} - h sum_i X_i

    to this sector, and dH is -sum_i X_i in the same basis.
    """
    Hzz, X, nrep = build_nnn_tfim_sector_components_k0_even(L, j2)
    H = (Hzz - h * X).tocsr()
    dH = (-X).tocsr()
    return H, dH, nrep
...
def sparse_gap_nnn(L: int, h: float, j2: float, tol: float = 1e-8) -> float:
    H, _ = build_nnn_tfim(L, h, j2)
...
def estimate_hc_prg_sector(L: int, j2: float, h_min: float = 0.8,
    ...
    Hs0, Xs, _ = build_nnn_tfim_sector_components_k0_even(L, j2)
    Hm0, Xm, _ = build_nnn_tfim_sector_components_k0_even(L - 2, j2)

    def f(h: float) -> float:
        return (L - 2) * _gap_from_components(Hm0, Xm, h) - L * _gap_from_components(Hs0, Xs, h)

    return bracketed_prg_root(f, h_min, h_max)


def estimate_hc_prg(L: int, j2: float, h_min: float = 0.8, h_max: float = 1.8) -> float:
    """Phenomenological RG: (L-2) Delta_{L-2} = L Delta_L."""
    if L < 6:
        raise ValueError("PRG estimate uses the pair (L-2,L), so L must be >= 6")

    Hs0, Xs = build_nnn_tfim_components(L, j2)
    Hm0, Xm = build_nnn_tfim_components(L - 2, j2)
...
def main() -> None:
    ...
    parser.add_argument("--nnn-j2", "--nnn-lambda", dest="nnn_j2", type=float, default=0.2,
                        help="single NNN-TFIM J2 value; --nnn-lambda is a legacy alias")
    parser.add_argument("--nnn-j2-values", "--nnn-lambdas", dest="nnn_j2_values", default="0,0.05,0.10,0.20",
    ...
    if args.nnn_h_table is not None:
        with args.nnn_h_table.open(newline="") as hf:
            for row in csv.DictReader(hf):
                serialized_coupling = row.get("J2") or row.get("lambda_or_g")
                h_table[(float(serialized_coupling), int(row["L"]))] = float(row["h"])
    if not models or not models <= {"potts", "nnn", "nnn-tfim"}:
        raise ValueError("unknown or empty model selection")
    if args.Lmin < 2 or args.Lmax < args.Lmin or args.nnn_Lmax < args.Lmin:
        raise ValueError("invalid size range")
    if "potts" in models and args.Lmax > 12:
        raise ValueError("Potts benchmark is capped at Lmax <= 12")
    if "potts" in models and args.potts_method == "full":
        check_dense_potts_budget(args.Lmax)
    if args.n_low_energy_eigenpairs < 2 or args.ncv_factor < 2 or any(
            not np.isfinite(t) or t <= 0 for t in (args.tol_eig, args.solve_rtol, args.level_group_tol)):
        raise ValueError("invalid solver settings")
    if args.benchmark_csv == args.weights_csv or any(
            Path(n).name != n or n in {"", ".", ".."} for n in (args.benchmark_csv, args.weights_csv)):
        raise ValueError("output names must be distinct basenames")
    couplings = [float(v) for v in args.nnn_j2_values.split(",") if v.strip()]
    parameters = [args.potts_g, args.nnn_j2, *couplings, *h_table.values()]
    if args.nnn_h is not None:
        parameters.append(args.nnn_h)
    if not all(np.isfinite(v) for v in parameters) or args.max_weight_ranks < 1:
        raise ValueError("nonfinite model parameter or invalid rank limit")
    args.outdir = fresh_output_directory(args.outdir or PROJECT / "build" / ("legacy-bench-" + uuid.uuid4().hex))
    ...
    with Path(str(bench_path) + ".partial").open("x", newline="") as bf, Path(str(weights_path) + ".partial").open("x", newline="") as wf:
            ...
            for L in range(args.Lmin, args.Lmax + 1):
                ...
                if args.potts_method == "full":
                    if L <= args.dense_potts_max:
                        ...
                        result = hybrid_level_projector_kf(Hs, dHs, args.n_low_energy_eigenpairs, tol_eig=args.tol_eig,
                                                           tol_group=args.level_group_tol,
                                                           rtol_solve=args.solve_rtol,
                                                           ncv_factor=args.ncv_factor, max_solve_residual=1e-8)
                        dim = 3**L
                        method = "full-hybrid"
                else:
        ...
        if "nnn" in models or "nnn-tfim" in models:
            if args.nnn_j2_values is None:
                j2_values = [args.nnn_j2]
            else:
                j2_values = [float(x) for x in args.nnn_j2_values.split(",") if x.strip()]
            Ls = parse_even_or_all_L(args.Lmin, args.nnn_Lmax, args.nnn_even_only)
            for j2 in j2_values:
                for L in Ls:
                    t0 = time.time()
                    if abs(j2) < 1e-14 and args.nnn_h is None:
                        h = 1.0
                        ...
                        if args.nnn_h is not None:
                            h = args.nnn_h
                            prg_note = "fixed-h"
                        elif (j2, L) in h_table:
                            h = h_table[(j2, L)]
                            prg_note = "tabulated full-PRG"
                        elif args.nnn_prg_method == "sector":
                            h = estimate_hc_prg_sector(L, j2)
                            prg_note = "sector-PRG"
                        else:
                            h = estimate_hc_prg(L, j2)
                            prg_note = "full-PRG"
                        if args.nnn_method == "sector":
                            Hs, dHs, dim_nnn = build_nnn_tfim_sector_k0_even(L, h, j2)
                            method = "PRG-sector-hybrid"
                        else:
                            Hs, dHs = build_nnn_tfim(L, h, j2)
                            dim_nnn = 1 << L
                            method = "PRG-hybrid"
                        result = hybrid_level_projector_kf(Hs, dHs, args.n_low_energy_eigenpairs, tol_eig=args.tol_eig,
                                                           tol_group=args.level_group_tol,
                    ...
                    write_benchmark_row(bench_writer, model="NNN-TFIM", L=L, dim=(1 << L) if abs(j2) < 1e-14 and args.nnn_h is None else dim_nnn,
                                        coupling=j2, h=h, result=result,
                                        method=method, seconds=seconds,
                                        note=note)
                    ...
                    print(f"NNN-TFIM L={L:2d} J2={j2:g} h={h:.8f} "
    ...
    Path(str(bench_path) + ".partial").rename(bench_path)
    Path(str(weights_path) + ".partial").rename(weights_path)
