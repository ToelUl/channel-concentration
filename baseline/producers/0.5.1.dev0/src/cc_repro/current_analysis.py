...
def replace_initial_benchmark(project, root, benchmark, rows):
    """Replace only the hash-verified fresh seed and retain its original bytes."""
    benchmark = evidence.reject_links(benchmark)
    seed = evidence.reject_links(project / "data/reproducibility/interacting_benchmarks.csv")
    saved = evidence.reject_links(root / "work/seed-reference/interacting_benchmarks.csv")
    initial = evidence.read(root / "context.json")["frozen_data_initial_files"].get("interacting_benchmarks.csv")
    if not benchmark.is_file() or initial != evidence.digest(seed) or evidence.digest(benchmark) != initial:
        raise ValueError("existing benchmark is not the verified initial seed")
    if saved.exists():
        raise FileExistsError("initial seed evidence already exists")
    if [r.get("L") for r in rows] != list(range(6, 12)) or any(
            r.get("model") != "Potts" or type(r.get("P2")) not in (int, float)
            or not math.isfinite(r["P2"]) or r["P2"] <= 0 for r in rows):
        raise ValueError("complete accepted six-point P2 set required")
    stream = io.StringIO()
    writer = csv.DictWriter(stream, fieldnames=["model", "L", "P2"])
    writer.writeheader(); writer.writerows(rows)
    saved.parent.mkdir(parents=True, exist_ok=True)
    with saved.open("xb") as handle:
        handle.write(benchmark.read_bytes())
    if evidence.digest(saved) != initial:
        raise ValueError("initial seed changed while preserving it")
    evidence.atomic_text(benchmark, stream.getvalue())
    return {"preserved_initial_seed": evidence.record(saved, root),
            "current_points_csv": evidence.record(benchmark, root)}
...
def p2_inputs(project, root):
    ...
    expected_identity = input_identity(state.get("identity"), evidence.context(project, recipe))
...
def run(project, root, analysis, input_run, ladder_run, p2_run):
    ...
    state, payloads = ladder.accepted(project, parent, allow_previous=True)
    inputs = [evidence.record(parent / "ladder.json", project.parent)]
    input_producers = {"ladder": state["identity"]["environment"]}
    ...
    if analysis == "projector-enclosures":
        ...
        expected = input_identity(fixed_state.get("plan"), expected)
        input_producers.update(p2=evidence.read(p2_root / "p2.json")["identity"]["environment"], fixed=expected["environment"])
        ...
        current_potts(project, fixed, staging, allow_previous=True)
        ...
        seed_projection = replace_initial_benchmark(project, root, benchmark, rows)
        ...
        evidence.write(root / "input-projection.json", {"source_role": "current", "inputs": inputs,
            ...
            "seed_replacement": seed_projection, "input_producer_environments": input_producers,
    ...
    evidence.commit(root, "postprocess", {"schema": "cc-repro-current-postprocess/1", "status": "PASS",
        ...
        "input_producer_environments": input_producers,
