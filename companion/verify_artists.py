...
def verify_artists(plot_dir,data,selectors=None):
    import numpy as np
    ...
    selected={'4','5'} if selectors is None else set(selectors) & {'4','5'}
    if '4' in selected:
    ...
    return {'status':'PASS','checked_figures':sorted(selected),'checks':checks,
            'scope':'Archived interacting data-coordinate binding for selected figures only; not spectral completeness or artwork approval.'}
