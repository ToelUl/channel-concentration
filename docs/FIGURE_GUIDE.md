...
This guide is the human route through Figures 1–6 and S1. The [machine figure map](../companion/FIGURE_MAP.json) binds selectors, renderers, filenames, direct inputs, caption roles, and the same code routes returned by `companion/run.py describe`; the [evidence guide](EVIDENCE_MAP.md) explains evidence classes; the [companion guide](../companion/README.md) explains the commands. The immutable numerical authority is `baseline-2026-09-09-rc1`. Figure numbering, labels, filenames, and caption roles were validated against the frozen final-local manuscript sources whose hashes are recorded in the figure map. Those source hashes are validation identities, not a claim that the manuscript prose is distributed by this repository.
